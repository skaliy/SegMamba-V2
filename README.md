...
This is a fork of [ge-xing/SegMamba-V2](https://github.com/ge-xing/SegMamba-V2) with two additions to `colorectal/models_segmamba/segmambav2.py`:

- **Flexible input size** — `MambaLayer` reshapes dynamically, so the model accepts any input whose spatial dimensions are divisible by 16 (rather than a single fixed size).
- **CPU inference** — pass `mamba_backend="mambamixer"` to run the Mamba blocks through HuggingFace `transformers.MambaMixer` instead of the CUDA-only `mamba_ssm` kernel. The two share an identical parameter layout, so a model trained with the default `mamba_ssm` backend loads into the CPU backend with `strict=True` (no retraining). The default backend (`"mamba_ssm"`) is unchanged.

## Installation

There is no `setup.py` — clone the repo and import the model directly (add the dataset folder to `sys.path`).

```bash
git clone https://github.com/skaliy/SegMamba-V2.git
cd SegMamba-V2
```

**Core dependencies** (model definition):

```bash
pip install torch einops monai
```

Then install the Mamba backend you need:

**GPU / training** — the default `mamba_ssm` backend (requires a CUDA toolchain):

```bash
pip install mamba-ssm causal-conv1d
```

**CPU inference** — the `mambamixer` backend (no CUDA, no `mamba_ssm` required):
...
```python
import sys, torch
sys.path.append("SegMamba-V2/colorectal")
from models_segmamba.segmambav2 import SegMamba

# GPU / training (default backend uses the mamba_ssm CUDA kernel)
model = SegMamba(in_chans=1, out_chans=2).cuda()

# CPU inference (transformers.MambaMixer); loads weights trained with the default backend
model = SegMamba(in_chans=1, out_chans=2, mamba_backend="mambamixer")
model.load_state_dict(torch.load("weights.pth", map_location="cpu"), strict=True)
model.eval()
```
